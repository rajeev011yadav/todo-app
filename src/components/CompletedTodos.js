...
const CompletedTodos = ({todos, remove_todo}) => {
    const getTodos = () => {
        let todosJSX = todos.filter((todo) => todo.completed === true && todo.deleted === false)
        ...
        return todosJSX.length > 0 ? todosJSX : <div className="text-muted text-center">No completed Todos</div>
    }
    return (
...
export default connect(mapStateToProps, mapDispathToState)(CompletedTodos);